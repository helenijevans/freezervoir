...
|     3 | Filters & Sorting                   | To make deletion smoother, add inbuilt filter and sorting for item IDs, default order earliest -> latest.                                                                                                                    |        | 
|     4 | QR-Driven Item Pages                | Generate QR codes that point to item detail pages, allowing delete actions via the UI. Will also see removal of Controller files/direct endpoints to prevent accidental data modification.                                   |        |
|     5 | Schema Migration & Physical Rollout | Replace the existing table with an improved schema and migrate all legacy data. Implement ID generation, assign QR codes to all items, and relabel every freezer item to align the real-world inventory with the new system. |        |
|     6 | UI Improvements + Notes Feature     | Display the current number of items, show and edit item status and notes on detail pages, and improve the UI layout and clarity.                                                                                             |        |
|     7 | Deployment                          | Deploy the application so the server runs continuously.                                                                                                                                                                      |        |
|     8 | Data Model Improvements             | Change IDs to be auto-generated and add a description field.                                                                                                                                                                 |        |
|     9 | App-Style Frontend                  | Improve the overall UI/UX and support viewing all items in a cleaner, more app-like interface.                                                                                                                               |        |
|    10 | QR Code Generation & Scanning       | Generate QR codes for new entries, integrate with the label maker workflow, and support scanning QR codes in-app to add and delete items from the database.                                                                  |        |
|    11 | Expiry Tracking                     | Notify when an item is approaching expiry (e.g. 1 year from date added).                                                                                                                                                     |        |
|    12 | UX & Workflow Cleanup               | Improve branding and overall UX, and refine the app → label workflow.                                                                                                                                                        |        |
|    13 | AI Enhancements                     | Suggest meals based on freezer contents. Replace manual data entry with camera-based capture. Use AI to suggest item descriptions and estimate realistic expiry dates.                                                       |        |
